...
The practical implementation phase yielded functioning technical pipelines with distinct outcomes across the rule-based and generative workflows:
...
\subsubsection*{LLM Pipeline and System Ingestion}
Advanced prompting of the Gemini 3.1 Pro (High) model successfully minimized clinical hallucinations and formatting inconsistencies, resulting in highly compliant JSON-based FHIR bundles. Because of the issues encountered with Synthea, the ingestion pipeline shifted its primary focus to these LLM-generated records. Integrating the comprehensive CIEL concept dictionary successfully resolved semantic gaps caused by missing concepts in the HIS database.

For laboratory data, the initial UI-simulating Python script succeeded in appending automated values, but was structurally limited as it required direct SQL database queries as a workaround to link laboratory orders to patient IDs, and it could not backdate historical timestamps. This limitation was resolved in the final architecture by utilizing the exact, low-level FHIR Observation schema accepted by the HIS. This allowed both laboratory measurements and continuous glucose sensor streams to be imported seamlessly via the regular FHIR interface as general observations, which are successfully visualized within the system's native trends dashboard.
