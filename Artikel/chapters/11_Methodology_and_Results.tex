...
\begin{description}[style=nextline, font=\bfseries\sffamily]
    \item[Methodik] 
        Die methodische Grundlage bildet eine systematische Literaturrecherche in den Datenbanken PubMed und IEEE Xplore (Stand: März 2026). Aus der Recherche resultierten $n=27$ relevante Quellen, die einer quantitativen Analyse sowie einer manuellen Kategorisierung hinsichtlich der eingesetzten Synthese-Engines und Frameworks unterzogen wurden.
    \item[Ergebnisse]
    ...
    \item[Eingesetzte Technologien] 
        ...
        \end{itemize}
    \item[Beantwortung]
        Die technologische Landschaft wird primär durch den Einsatz von Large Language Models (LLMs) und Generative Adversarial Networks (GANs) geprägt, ergänzt durch regelbasierte, agentenbasierte Systeme wie das Synthea-Framework.
\end{description}
...
\begin{description}[style=nextline, font=\bfseries]
    ...
    \item[Methodik] 
    Die methodische Grundlage bildet eine systematische Literaturrecherche in PubMed und IEEE Xplore (Stand: März 2026). Die $n = 14$ identifizierten Quellen wurden quantitativ analysiert und manuell hinsichtlich der verwendeten Schnittstellenstandards kategorisiert.
...
\subsubsection{Frage 2.1: Anforderungen von Experten}
...
\\
    ...
    \vspace{1em}
    Im Anschluss an die Ausarbeitung des Leitfadens erfolgte die Durchführung der eigentlichen Experteninterviews. Hierbei wurde ein zielgruppenadaptierter Ansatz gewählt, um die Befragung optimal an die jeweilige Fachexpertise der Interviewpartner anzupassen. Dies bedeutet konkret, dass nicht jedem Experten der vollständige Fragenkatalog vorgelegt wurde: Personen mit primär medizinischem Hintergrund wurden nicht zu informationstechnischen Details befragt, ebenso wie technische Experten von den spezifisch medizinischen Qualitätskriterien ausgenommen waren. Auf diese Weise konnten insgesamt fünf qualitative Interviews realisiert werden. Die Kohorte der Befragten setzte sich aus vier Personen mit medizinischem Hintergrund sowie einer Person mit informationstechnischer Spezialisierung zusammen. Ein essenzielles Auswahlkriterium, das alle fünf Experten gleichermaßen erfüllten, war ihre ausgewiesene praktische Erfahrung im Bereich der akademischen Lehre.
\item[Ergebnisse] Die qualitative Analyse der Interviewprotokolle führte zur Identifikation von vier primären Hauptkriterien für die Datenqualität:
\begin{enumerate}
...
\item[Beantwortung] Die Ermittlung der Expertenanforderungen erfolgte über fünf qualitative, zielgruppenadaptierte Interviews auf Basis eines dreigeteilten Leitfadens (Medizin, IT, Didaktik). Als zentrales Ergebnis konnten vier  Kernkriterien, klinische Plausibilität, longitudinale Konsistenz, Zeitlich logische Abfolge und der Gesamteindruck extrahiert werden, welche die qualitativen Mindestanforderungen an die synthetischen Patientendaten definieren.
\end{description}
...
\begin{description}[style=nextline, font=\bfseries]
    ...
    \begin{itemize}[leftmargin=1.5em, labelsep=0.5em, itemsep=4pt, topsep=4pt]
        ...
        Synthea erstellt für einen Patienten immer eine Liste an Ärzten die diesen über den Verlauf behandelt haben. Dies würde für das KIS bedeuten, das sich die Menge der Ärzte exponentiell erhöhen würde, da jeder Patient eine vielzahl an Ärzten hat die Ihn über den Verlauf behandeln. Ein Lösungsansatz wird in Frage 3.c. dargestellt.
    \end{itemize}
