    ...
    for file_path in json_files:
        ...
        for entry in bundle.get('entry', []):
            ...
            def get_ref(field_name):
                return resource.get(field_name, {}).get('reference', '').replace('urn:uuid:', '').replace('Patient/', '').replace('Encounter/', '')
            ...
            if res_type == 'Patient':
                ...
                patients_data.append({
                    ...
                    'ADDRESS': address_line, 'CITY': city, 'STATE': state, 'ZIP': zip_code,
            ...
            # ---------------- ALLERGIES ----------------
                ...
                allergies_data.append({
                    ...
                    'PATIENT': resource.get('patient', {}).get('reference', '').replace('urn:uuid:', ''),
                    'ENCOUNTER': get_ref('encounter'),
                    'CODE': coding.get('code', ''),
                    'SYSTEM': coding.get('system', ''),
                    'DESCRIPTION': coding.get('display', ''),
                    ...
                    'DESCRIPTION1': manifestation.get('display', ''),
            ...
            # ---------------- IMAGING STUDIES ----------------
                ...
                imaging_studies_data.append({
                    ...
                    'BODYSITE_DESCRIPTION': body_site.get('display', ''),
                    'MODALITY_CODE': modality.get('code', ''),
                    'MODALITY_DESCRIPTION': modality.get('display', ''),
